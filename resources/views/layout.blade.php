...
<body class="min-h-screen">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function(){
            ...
            // Intercept sidebar link clicks and fetch page content via AJAX
            // Send header X-Internal-Navigation=1 so middleware allows the request
            // We DON'T update the browser URL to keep direct URL navigation blocked
            // ==========================
            ...
            function fetchAndReplace(href, clickedLink) {
                fetch(href, { headers: { 'X-Internal-Navigation': '1' }, credentials: 'same-origin' })
                    .then(r => {
                        if (r.redirected) {
                            // server redirected (probably blocked) - follow by full navigation
                    ...
                    .then(html => {
                        ...
                        const newMain = doc.querySelector('main .max-w-7xl');
                        const currentMain = document.querySelector('main .max-w-7xl');
                        if (newMain && currentMain) {
                            currentMain.innerHTML = newMain.innerHTML;
                            // update active class on sidebar
                            document.querySelectorAll('.sidebar-link').forEach(s => s.classList.remove('active'));
                            if (clickedLink) clickedLink.classList.add('active');
                            // run any inline scripts from the fetched content (simple approach)
