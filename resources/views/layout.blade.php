...
<body class="min-h-screen">
    ...
    <nav class="fixed top-0 right-0 left-0 md:left-64 bg-white border-b border-gray-200 z-30 h-16">
        <div class="h-full px-4 flex items-center justify-end gap-4">
            @auth
                @if(auth()->user()->role == 'admin' || auth()->user()->role == 'petugas')
                    <!-- Notification Bell Icon for Admin/Petugas -->
                    <a href="{{ route('home') }}" class="relative p-2 rounded-lg hover:bg-gray-100 transition-colors" onclick="event.preventDefault(); window.location.href='{{ route('home') }}'; setTimeout(() => { const notifSection = document.getElementById('notifications'); if(notifSection) notifSection.scrollIntoView({behavior: 'smooth', block: 'start'}); }, 300);">
                ...
                @else
                    <!-- Message Icon for Regular Users -->
                    <a href="{{ route('messages.my') }}" class="relative p-2 rounded-lg hover:bg-gray-100 transition-colors">
                        <svg class="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                        </svg>
                        @php
                            $unreadMessagesCount = \App\Models\Message::where('from_user_id', auth()->id())
                                ->where(function($q) {
                                    $q->whereNotNull('reply')->where('is_read', false);
                                })
                                ->count();
                        @endphp
